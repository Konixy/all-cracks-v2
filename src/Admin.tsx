...
export default function Init() {
  ...
  function login(event: FormEvent<HTMLFormElement>) {
    ...
    if (data) {
      axios.post(`${config.backendPath}/api/admin/login`, data, { withCredentials: false }).then((r: AxiosResponse<AdminAPIResponse>) => {
        if (r.data.success) {
  ...
  function checkConnected() {
    axios.get(`${config.backendPath}/api/admin/info`, { withCredentials: true }).then((r: AxiosResponse<AdminAPIResponse>) => {
      if (r.data.success) {
...
function Admin(): JSX.Element {
  ...
  return (
    <div className="my-20 mx-10 flex flex-col items-center justify-center text-center">
...
function DropDown({ options }: { options: APIGame[] }) {
  ...
  query === ''
    ...
    : (filteredPeople = options.filter((e) => e.name.toLowerCase().replace(/\s+/g, '').includes(query.toLowerCase().replace(/\s+/g, ''))));

  return (
    <div className="top-16 block w-72">
            ...
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {filteredPeople.length === 0 && query !== '' ? (
                ...
                filteredPeople.map((person) => (
                  <Combobox.Option
                    key={person._id}
                    className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-sky-600 text-white' : 'text-gray-900'}`}
                    value={person}
                  >
                    {({ selected, active }) => (
                      <>
